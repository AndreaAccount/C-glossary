...
int main(void)
{
    ...
    do
	{
		...
		char choice;
		int lsel;
		printf("%s", "C GLOSSARY\nInstructions' syntax list, their function and various definitions\n\n----------------------------------------------\n");
		...
		if(choice == 'L')
		{
			...
			printf ("%s", "Which one do you choose? Type the number: ");
			scanf(" %i", &lsel);
		}
		switch(lsel)
		{
		...
		if(end != 'N' && end != 'Y')
		{
			do
			{
				printf("ERROR: invalid option. Please, try again. (Y/N): ");
				scanf(" %c", &end);
				end = toupper(end);
				if(end == 'N')
					return 0;
